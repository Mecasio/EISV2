...
app.post("/login_prof", async (req, res) => {
  ...
  try {
    ...
    const token = webtoken.sign({ person_id: user.person_id, email: user.email, role: user.role }, process.env.JWT_SECRET, { expiresIn: "24h" });
...
app.post("/adding_course", async (req, res) => {
  ...
  try {
    ...
    await db3.query(
      ...
      (course_code, course_description, course_unit, lab_unit, prereq, corequisite, iscomputer_lab, isnon_computer_lab, is_nstp)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        normalized_code,
...
app.put("/update_course/:id", async (req, res) => {
  ...
  try {
    ...
    const final_course_code = course_code
      ? course_code.replace(/[^A-Za-z0-9]/g, "").toUpperCase()
      : current.course_code;
    ...
    const [result] = await db3.query(
      ...
      [
        ...
        iscomputer_lab ?? current.iscomputer_lab,
        isnon_computer_lab ?? current.isnon_computer_lab,
        is_nstp ?? current.is_nstp,
        id,
      ]
    );
...
// ✅ UPDATE COURSE REQUIREMENTS ONLY
app.put("/update_course_requirements/:id", async (req, res) => {
  const { id } = req.params;
  ...
  try {
    ...
    console.error("❌ Error updating course requirements:", err);
...
app.get("/api/course/:courseId/fees", async (req, res) => {
  const { courseId } = req.params;

  try {
    const [[course]] = await db3.query(
      `SELECT iscomputer_lab, isnon_computer_lab, is_nstp
       FROM course_table
       WHERE course_id = ?`,
      [courseId]
    );
    ...
    if (course.iscomputer_lab === 1) feeCodes.push("LAB_BASE");
    if (course.isnon_computer_lab === 1) feeCodes.push("COMPUTER_FEE");
    if (course.is_nstp === 1) feeCodes.push("NSTP_FEE");
...
app.post("/insert_fee_rule", async (req, res) => {
  const { fee_code, description, amount } = req.body;

  if (!fee_code || !description || amount == null) {
    return res.status(400).json({ message: "All fields are required" });
  }

  try {
    await db3.query(
      `INSERT INTO fee_rules (fee_code, description, amount) VALUES (?, ?, ?)`,
      [fee_code, description, amount]
    );
...
app.put("/update_fee_rule/:fee_code", async (req, res) => {
  ...
  try {
    const [result] = await db3.query(
      `UPDATE fee_rules SET description = ?, amount = ? WHERE fee_code = ?`,
      [description, amount, fee_code]
    );
    ...
    res.json({ message: "Fee rule updated successfully" });
  } catch (err) {
    console.error(err);
    res.status(500).json({ message: "Failed to update fee rule" });
  }
});
...
app.get("/api/search-student/:sectionId", async (req, res) => {
  ...
  FROM curriculum_table ct
  INNER JOIN program_table pt ON ct.program_id = pt.program_id
  INNER JOIN course_table c ON c.program_id = pt.program_id
  ...
  `,
    ...
    const formattedCourses = courses.map(c => ({
      ...c,
      prereq_list: c.prereq
        ? c.prereq.split(",").map(p => p.trim())
        : []
    }));
